...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only){
	...
	} else if(apr_fnmatch(DEPENDS_PAT, info.patch, 0)==APR_SUCCESS){
	...
	} else if(apr_fnmatch(TAR_BZ2_PAT, info.patch, 0)==APR_SUCCESS){
		if(info.scheme){
	...
	const char *make_opts, const char *install_opts)
{
	...
	check(access(BUILD_DIR, X-OK | R_OK | W_OK)==0,
	...
	chck(rc==0, "Failed to build.");
	
	rc=SHell_exec(INSTALL_SH,
		"TARGET", install_opts ? install opts: "install",
		NULL);
...
int Command_install(apr_pool_t *p, const char *url, const char *configure_opts,
	const char *make_opts, const char *install opts){
